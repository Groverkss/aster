//===- DecomposePtrOffset.cpp - Decompose ptr.ptr_add offsets -------------===//
//
...
#define DEBUG_TYPE "decompose-ptr-offset"

namespace mlir::aster {
namespace aster_utils {
#define GEN_PASS_DEF_DECOMPOSEPTROFFSET
...
namespace {
//===----------------------------------------------------------------------===//
// DecomposePtrOffset pass
//===----------------------------------------------------------------------===//

struct DecomposePtrOffset
    : public aster_utils::impl::DecomposePtrOffsetBase<DecomposePtrOffset> {
public:
  using Base::Base;
...
void OffsetComponents::Offsets::mul(const Offsets &other) {
  // Full distributive multiplication:
  // (c1 + u1 + d1) * (c2 + u2 + d2)
  // const:   c1*c2
  // uniform: c1*u2 + u1*c2 + u1*u2
  // dynamic: c1*d2 + u1*d2 + d1*c2 + d1*u2 + d1*d2
  ...
  AffineExpr newDynamic = constOffset * other.dynamicOffset +
                          uniformOffset * other.dynamicOffset +
                          dynamicOffset * other.constOffset +
...
AffineExpr OffsetComponents::getAsExpr(Value value) {
  ...
  if (std::optional<APInt> constVal = getConstantValue(value, solver))
    return getAffineConstantExpr(constVal->getZExtValue(), context);
...
OffsetComponents::analyzeTerm(Value value) {
  ...
  if (std::optional<APInt> constVal = getConstantValue(value, solver)) {
    ...
    return Offsets(getAffineConstantExpr(constVal->getZExtValue(), context),
                   getAffineConstantExpr(0, context),
                   getAffineConstantExpr(0, context));
  }
...
static void optimizePtrAddOp(ptr::PtrAddOp op, DataFlowSolver &solver) {
  // TODO: Support decomposition for ptr_add ops without overflow flags.
...
void DecomposePtrOffset::runOnOperation() {
  ...
  SmallVector<ptr::PtrAddOp> ptrAddOps;
  op->walk([&](ptr::PtrAddOp ptrAddOp) { ptrAddOps.push_back(ptrAddOp); });
  for (auto ptrAddOp : ptrAddOps)
    optimizePtrAddOp(ptrAddOp, solver);
}
