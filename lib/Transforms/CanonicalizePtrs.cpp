...
ExpandTypeOffsetOpPattern::matchAndRewrite(ptr::TypeOffsetOp op,
  ...
  if (!typeSize.isFixed())
    return rewriter.notifyMatchFailure(op, "scalable type size cannot be folded to a constant");
  ...
  auto newValue = arith::ConstantOp::create(rewriter, op.getLoc(), offsetAttr);
  rewriter.replaceOp(op, newValue.getResult());
  return success();
}
...
PtrAddOpPattern::matchAndRewrite(ptr::PtrAddOp op,
  ...
  if (!baseOp)
    return rewriter.notifyMatchFailure(op, "base is not defined by a ptr.ptr_add");
  if (baseOp.getOffset().getType() != op.getOffset().getType())
    return rewriter.notifyMatchFailure(op, "offset types differ between chained ptr_add ops");
...
PtrAddZeroOpPattern::matchAndRewrite(ptr::PtrAddOp op,
  ...
  if (Operation *cOp = op.getOffset().getDefiningOp();
    ...
    return rewriter.notifyMatchFailure(op, "offset is not a constant zero");
...
void CanonicalizePtrOps::runOnOperation() {
  ...
  MLIRContext *context = &getContext();
  {
    ...
    for (RegisteredOperationName op :
         context->getRegisteredOperationsByDialect(arithDialect->getNamespace()))
      op.getCanonicalizationPatterns(patterns, context);
  }
  {
    auto *affineDialect = context->getLoadedDialect<affine::AffineDialect>();
    affineDialect->getCanonicalizationPatterns(patterns);
    for (RegisteredOperationName op :
         context->getRegisteredOperationsByDialect(affineDialect->getNamespace()))
      op.getCanonicalizationPatterns(patterns, context);
  }
